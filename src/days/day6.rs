...
mod tests {
    ...
    fn day6_example() {
        ...
        let input = io::read_string_col(Path::new("data/day6/example.txt")).expect("Unable to find input file. ");
        
        let outputs: Vec<usize> = input.iter().map(|input_str| {
            input_str.chars().collect::<Vec<char>>().windows(target_window_size).position(|wind| {
                let mut tmp: Vec<char> = wind.clone().into();
                tmp.sort();
                tmp.dedup();
                tmp.len() == target_window_size
            }).expect("Valid window not found in input. ") + 4
        }).collect();
    ...
    fn day6_part1() {
        ...
        let target_window_size: usize = 4;
        let input = io::read_string_col(Path::new("data/day6/data.txt")).expect("Unable to find input file. ");
        ...
        let outputs: Vec<usize> = input.iter().map(|input_str| {
            ...
            input_str.chars().collect::<Vec<char>>().windows(target_window_size).position(|wind| {
                ...
                tmp.dedup();
                tmp.len() == target_window_size
            }).expect("Valid window not found in input. ") + target_window_size // Add on here to compensate for widnow size. 
        }).collect();
