...
impl Command {
    pub fn from_str(input: &str) -> Command {
        ...
        match segments.first().expect("Empty string for input command. ") {
            &"noop" => Self::Noop,
            &"addx" => {
                let operand = segments[1].parse::<i32>().expect("Addx operand is not an integer. ");
                Self::Addx(operand)
            },
...
#[cfg(test)]
mod tests {
    use super::{Command, value_at_cycle};
    use crate::io;
    use std::path::Path;

    #[test]
    pub fn day10_example() {
        let input_vec: Vec<String> = io::read_string_col(Path::new("data/day10/example.txt")).expect("No input found. ");
        let commands: Vec<Command> = input_vec.iter().map(|string| Command::from_str(string.as_str())).collect();
        let x: i32 = 1;
        ...
        assert_eq!(c20 * 20 + c60 * 60 + c100 * 100 + c140 * 140 + c180 * 180 + c220 * 220, 13140);
    }

    #[test]
    pub fn day10_part1() {
        let input_vec: Vec<String> = io::read_string_col(Path::new("data/day10/data.txt")).expect("No input found. ");
        let commands: Vec<Command> = input_vec.iter().map(|string| Command::from_str(string.as_str())).collect();
        let x: i32 = 1;
