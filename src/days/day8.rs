...
impl TreeMap {
    ...
    pub fn on_boundary(&self, i: usize, j: usize) -> bool {
        if i == 0 || j == 0 || i == self.heights.len() - 1|| j == self.heights.first().expect("Unable to find first row. ").len() - 1 {
             true
        } else {
            false
        }
    ...
    pub fn count_visible(&self) -> usize {
        ...
        for i in 0..self.heights.len() {
            for j in 0..self.heights.first().expect("Unable to find first row. ").len() {
