...
mod tests {
    ...
    fn day4_example() {
        ...
        let range1: Vec<Range<usize>> = col1.iter().map(|str| {
            let components: (usize, usize) = str.split("-").map(|comp| comp.parse::<usize>().unwrap() ).into_iter().collect_tuple().unwrap();
            (components.0..components.1)
        }).collect();

        let range2: Vec<Range<usize>> = col2.iter().map(|str| {
            let components: (usize, usize) = str.split("-").map(|comp| comp.parse::<usize>().unwrap() ).into_iter().collect_tuple().unwrap();
            (components.0..components.1)
        }).collect();

        let num_contains: usize = range1.into_iter().zip(range2).map(|(r1, r2)| {
            if contains(&r1, &r2) || contains(&r2, &r1) { 1 } else { 0 }
        }).sum();
    ...
    fn day4_part2() {
        ...
        let num_contains: usize = range1.into_iter().zip(range2).map(|(r1, r2)| {
            if overlaps(&r1, &r2) || overlaps(&r2, &r1) { 1 } else { 0 }
        }).sum();
