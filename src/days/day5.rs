...
impl Rearrangement {
    pub fn from_string(in_string: &String) -> Result<Rearrangement, Error> {
        ...
        Ok(Rearrangement { from_stack, to_stack, n })
    }
    ...
    pub fn rearrange_multiple(&self, stacks: &mut Vec<Vec<char>>) {
        let tmp = (0..self.n).map(|_| stacks[self.from_stack - 1].pop().unwrap() ).collect::<Vec<char>>();
...
pub fn parse_rearrangements(lines: &Vec<String>) -> Vec<Rearrangement> {
    lines.iter().map(|line| Rearrangement::from_string(line).unwrap()).collect::<Vec<Rearrangement>>()
}
...
pub fn parse_stacks(lines: &Vec<String>) -> Vec<Vec<char>> {
    ...
    let parsed_lines: Vec<Vec<Option<char>>> = lines.iter().rev().skip(1).map(|line| {
        line.chars().chunks(4).into_iter().fold(Vec::<Option<char>>::new(), |mut stack, chars| {
    ...
    let mut stacks: Vec<Vec<char>> = (0..parsed_lines.first().unwrap().len()).into_iter().map(|_| Vec::new()).collect_vec();
...
pub fn parse_input(lines: &Vec<String>) -> (Vec<Vec<char>>, Vec<Rearrangement>) {
    ...
    (parse_stacks(&stack_lines), parse_rearrangements(&rearrange_lines))
}
...
mod tests {
    ...
    fn day5_example() {
        ...
        let outstr = stacks.into_iter().map(|curr_stack| *curr_stack.last().as_ref().unwrap().clone() ).collect::<String>();
